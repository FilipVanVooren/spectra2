...
0006               *
0007               ***************************************************************
0008               * File: fio.asm                     ; Version 191104-30488
0009               *--------------------------------------------------------------
...
0052 6012 71EE             data  runlib
0053               ;       data  haltme
0055               
0056 6014 1546             byte  21
0057 6015 ....             text  'FIO TEST 191104-30488'
0058                       even
0059               
0067               *--------------------------------------------------------------
...
0048               ;                                   ;         Bit 13-15 used by DSR for returning
0049               ;                                   ;         file error details to DSRLNK
0050               ;       data  vrecbuf               ;  2-3  - Record buffer in VDP memory
0051               ;       byte  80                    ;  4    - Record length (80 characters maximum)
0052               ;       byte  0                     ;  5    - Character count (bytes read)
0053               ;       data  >0000                 ;  6-7  - Seek record (only for fixed records)
0054               ;       byte  >00                   ;  8    - Screen offset (cassette DSR only)
0055               ; -------------------------------------------------------------
0056               ;       byte  11                    ;  9    - File descriptor length
0057               ;       text 'DSK1.MYFILE'          ; 10-.. - File descriptor (Device + '.' + File name)
0058               ;       even
0059               ***************************************************************
0060               
0061               
0062               
0063               ***************************************************************
0064               * file.open - Open File for procesing
0065               ***************************************************************
0066               *  bl   @file.open
0067               *  data P0
0068               *--------------------------------------------------------------
0069               *  P0 = Address of PAB in VDP RAM
0070               *--------------------------------------------------------------
0071               *  bl   @xfile.open
0072               *
0073               *  R0 = Address of PAB in VDP RAM
0074               ********@*****@*********************@**************************
0075               file.open:
0076 70A6 C03B  30         mov   *r11+,r0              ; Get file descriptor (P0)
0077               *--------------------------------------------------------------
0078               * Initialisation
0079               *--------------------------------------------------------------
0080               xfile.open:
0081 70A8 C04B  18         mov   r11,r1                ; Save return address
0082 70AA C100  18         mov   r0,tmp0               ; VDP write address (PAB byte 0)
0083 70AC 04C5  14         clr   tmp1                  ; io.op.open
0084 70AE 06A0  32         bl    @xvputb               ; Write file opcode to VDP
     70B0 611A 
0085               file.open_init:
0086 70B2 0220  22         ai    r0,9                  ; Move to file descriptor length
     70B4 0009 
0087 70B6 C800  38         mov   r0,@>8356             ; Pass file descriptor to DSRLNK
     70B8 8356 
0088               *--------------------------------------------------------------
0089               * Main
0090               *--------------------------------------------------------------
0091               file.open_main:
0092 70BA 0420  54         blwp  @dsrlnk               ; Call DSRLNK
     70BC 6F90 
0093 70BE 0008             data  8                     ; Level 2 IO
0094               *--------------------------------------------------------------
0095               * Check if error occured during file open operation
0096               *--------------------------------------------------------------
0097 70C0 1333  14         jeq   file.error            ; Jump to error handler
0098               *--------------------------------------------------------------
0099               * Exit
0100               *--------------------------------------------------------------
0101               file.open_exit:
0102 70C2 1027  14         jmp   file.record.statusbyte
0103                                                   ; Get status and return to caller
0104               
0105               
0106               
0107               ***************************************************************
0108               * file.close - Close currently open file
0109               ***************************************************************
0110               *  bl   @file.close
0111               *  data P0
0112               *--------------------------------------------------------------
0113               *  P0 = Address of PAB in VDP RAM
0114               *--------------------------------------------------------------
0115               *  bl   @xfile.close
0116               *
0117               *  R0 = Address of PAB in VD RAM
0118               ********@*****@*********************@**************************
0119               file.close:
0120 70C4 C03B  30         mov   *r11+,r0              ; Get file descriptor (P0)
0121               *--------------------------------------------------------------
0122               * Initialisation
0123               *--------------------------------------------------------------
0124               xfile.close:
0125 70C6 C04B  18         mov   r11,r1                ; Save return address
0126 70C8 C100  18         mov   r0,tmp0               ; VDP write address (PAB byte 0)
0127 70CA 0205  20         li    tmp1,io.op.close      ; io.op.close
     70CC 0001 
0128 70CE 06A0  32         bl    @xvputb               ; Write file opcode to VDP
     70D0 611A 
0129               file.close_init:
0130 70D2 0220  22         ai    r0,9                  ; Move to file descriptor length
     70D4 0009 
0131 70D6 C800  38         mov   r0,@>8356             ; Pass file descriptor to DSRLNK
     70D8 8356 
0132               *--------------------------------------------------------------
0133               * Main
0134               *--------------------------------------------------------------
0135               file.close_main:
0136 70DA 0420  54         blwp  @dsrlnk               ; Call DSRLNK
     70DC 6F90 
0137 70DE 0008             data  8                     ;
0138               *--------------------------------------------------------------
0139               * Check if error occured during file open operation
0140               *--------------------------------------------------------------
0141 70E0 1323  14         jeq   file.error            ; Jump to error handler
0142               *--------------------------------------------------------------
0143               * Exit
0144               *--------------------------------------------------------------
0145               file.close_exit:
0146 70E2 1017  14         jmp   file.record.statusbyte
0147                                                   ; Get status and return to caller
0148               
0149               
0150               
0151               
0152               
0153               ***************************************************************
0154               * file.record.read - Read record from file
0155               ***************************************************************
0156               *  bl   @file.record.read
0157               *  data P0
0158               *--------------------------------------------------------------
0159               *  P0 = Address of PAB in VDP RAM (without +9 offset!)
0160               *--------------------------------------------------------------
0161               *  bl   @xfile.record.read
0162               *
0163               *  R0 = Address of PAB in VDP RAM
0164               ********@*****@*********************@**************************
0165               file.record.read:
0166 70E4 C03B  30         mov   *r11+,r0              ; Get file descriptor (P0)
0167               *--------------------------------------------------------------
0168               * Initialisation
0169               *--------------------------------------------------------------
0170               xfile.record.read:
0171 70E6 C04B  18         mov   r11,r1                ; Save return address
0172 70E8 C100  18         mov   r0,tmp0               ; VDP write address (PAB byte 0)
0173 70EA 0205  20         li    tmp1,io.op.read       ; io.op.read
     70EC 0002 
0174 70EE 06A0  32         bl    @xvputb               ; Write file opcode to VDP
     70F0 611A 
0175               file.record.read_init:
0176 70F2 0220  22         ai    r0,9                  ; Move to file descriptor length
     70F4 0009 
0177 70F6 C800  38         mov   r0,@>8356             ; Pass file descriptor to DSRLNK
     70F8 8356 
0178               *--------------------------------------------------------------
0179               * Main
0180               *--------------------------------------------------------------
0181               file.record.read_main:
0182 70FA 0420  54         blwp  @dsrlnk               ; Call DSRLNK
     70FC 6F90 
0183 70FE 0008             data  8                     ;
0184               *--------------------------------------------------------------
0185               * Check if error occured during file open operation
0186               *--------------------------------------------------------------
0187 7100 1313  14         jeq   file.error            ; Jump to error handler
0188               *--------------------------------------------------------------
0189               * Exit
0190               *--------------------------------------------------------------
0191               file.record.read_exit:
0192 7102 1007  14         jmp   file.record.statusbyte
0193                                                   ; Get status and return to caller
0194               
0195               
0196               
0197               
0198               file.record.write:
0199 7104 1000  14         nop
0200               
0201               
0202               file.record.seek:
0203 7106 1000  14         nop
0204               
0205               
0206               file.image.load:
0207 7108 1000  14         nop
0208               
0209               
0210               file.image.save:
0211 710A 1000  14         nop
0212               
0213               
0214               file.delete:
0215 710C 1000  14         nop
0216               
0217               
0218               file.rename:
0219 710E 1000  14         nop
0220               
0221               
0222               file.status:
0223 7110 1000  14         nop
0224               
0225               
0226               
0227               ***************************************************************
0228               * file.record.statusbyte - Return PAB statusbyte to caller
0229               ********@*****@*********************@**************************
0230               file.record.statusbyte:
0231 7112 02C6  12         stst  tmp2                  ; Store status register contents in tmp2
0232                                                   ; Upon DSRLNK return status register EQ bit
0233                                                   ; 1 = No file error
0234                                                   ; 0 = File error occured
0235               *--------------------------------------------------------------
0236               * Get PAB status byte from VDP ram
0237               *--------------------------------------------------------------
0238 7114 C120  34         mov   @>8356,tmp0           ; Get PAB VDP address + 9
     7116 8356 
0239 7118 0224  22         ai    tmp0,-8               ; Get address of PAB + 1
     711A FFF8 
0240 711C 06A0  32         bl    @xvgetb               ; VDP read PAB status byte into tmp0
     711E 6128 
0241               *--------------------------------------------------------------
0242               * Check if error occured during file open operation
0243               *--------------------------------------------------------------
0244 7120 21A0  38         coc   @wbit2,tmp2           ; Equal bit set?
     7122 6046 
0245 7124 1301  14         jeq   file.error            ; Jump to error handler
0246 7126 0451  20         b     *r1                   ; Return to caller
0247               
0248               
0249               
0250               ***************************************************************
0251               * file.error - Error handler for file errors
0252               ********@*****@*********************@**************************
0253               file.error:
0254               ;
0255               ; When errors do occur then equal bit in status register is set (1)
0256               ; If no errors occur, then equal bit in status register is reset (0)
0257               ;
0258               ; So upon returning from DSRLNK in your file handling code you
0259               ; should basically add:
0260               ;
0261               ;       jeq   file.error            ; Jump to error handler
0262               ;
0263               *--------------------------------------------------------------
0264 7128 0460  28         b     @crash_handler        ; A File error occured
     712A 6050 
**** **** ****     > runlib.asm
0201               
0202               
...
0019               ********@*****@*********************@**************************
0020 712C 0300  24 tmgr    limi  0                     ; No interrupt processing
     712E 0000 
0021               *--------------------------------------------------------------
0022               * Read VDP status register
0023               *--------------------------------------------------------------
0024 7130 D360  34 tmgr1   movb  @vdps,r13             ; Save copy of VDP status register in R13
     7132 8802 
0025               *--------------------------------------------------------------
0026               * Latch sprite collision flag
0027               *--------------------------------------------------------------
0028 7134 2360  38         coc   @wbit2,r13            ; C flag on ?
     7136 6046 
0029 7138 1602  14         jne   tmgr1a                ; No, so move on
0030 713A E0A0  34         soc   @wbit12,config        ; Latch bit 12 in config register
     713C 6032 
0031               *--------------------------------------------------------------
0032               * Interrupt flag
0033               *--------------------------------------------------------------
0034 713E 2360  38 tmgr1a  coc   @wbit0,r13            ; Interupt flag set ?
     7140 604A 
0035 7142 1311  14         jeq   tmgr4                 ; Yes, process slots 0..n
0036               *--------------------------------------------------------------
...
0046               *--------------------------------------------------------------
0047 7144 20A0  38 tmgr2   coc   @wbit8,config         ; Kernel thread blocked ?
     7146 603A 
0048 7148 1305  14         jeq   tmgr3                 ; Yes, skip to user hook
0049 714A 20A0  38         coc   @wbit9,config         ; Kernel thread enabled ?
     714C 6038 
0050 714E 1602  14         jne   tmgr3                 ; No, skip to user hook
0051 7150 0460  28         b     @kthread              ; Run kernel thread
     7152 71CA 
0052               *--------------------------------------------------------------
0053               * Run user hook
0054               *--------------------------------------------------------------
0055 7154 20A0  38 tmgr3   coc   @wbit6,config         ; User hook blocked ?
     7156 603E 
0056 7158 13EB  14         jeq   tmgr1
0057 715A 20A0  38         coc   @wbit7,config         ; User hook enabled ?
     715C 603C 
0058 715E 16E8  14         jne   tmgr1
0059 7160 C120  34         mov   @wtiusr,tmp0
     7162 832E 
0060 7164 0454  20         b     *tmp0                 ; Run user hook
0061               *--------------------------------------------------------------
0062               * Do internal housekeeping
0063               *--------------------------------------------------------------
0064 7166 40A0  34 tmgr4   szc   @tmdat,config         ; Unblock kernel thread and user hook
     7168 71C8 
0065 716A C10A  18         mov   r10,tmp0
0066 716C 0244  22         andi  tmp0,>00ff            ; Clear HI byte
     716E 00FF 
0067 7170 20A0  38         coc   @wbit2,config         ; PAL flag set ?
     7172 6046 
0068 7174 1303  14         jeq   tmgr5
0069 7176 0284  22         ci    tmp0,60               ; 1 second reached ?
     7178 003C 
0070 717A 1002  14         jmp   tmgr6
0071 717C 0284  22 tmgr5   ci    tmp0,50
     717E 0032 
0072 7180 1101  14 tmgr6   jlt   tmgr7                 ; No, continue
0073 7182 1001  14         jmp   tmgr8
0074 7184 058A  14 tmgr7   inc   r10                   ; Increase tick counter
0075               *--------------------------------------------------------------
0076               * Loop over slots
0077               *--------------------------------------------------------------
0078 7186 C120  34 tmgr8   mov   @wtitab,tmp0          ; Pointer to timer table
     7188 832C 
0079 718A 024A  22         andi  r10,>ff00             ; Use R10LB as slot counter. Reset.
     718C FF00 
0080 718E C1D4  26 tmgr9   mov   *tmp0,tmp3            ; Is slot empty ?
0081 7190 1316  14         jeq   tmgr11                ; Yes, get next slot
0082               *--------------------------------------------------------------
0083               *  Check if slot should be executed
0084               *--------------------------------------------------------------
0085 7192 05C4  14         inct  tmp0                  ; Second word of slot data
0086 7194 0594  26         inc   *tmp0                 ; Update tick count in slot
0087 7196 C194  26         mov   *tmp0,tmp2            ; Get second word of slot data
0088 7198 9820  54         cb    @tmp2hb,@tmp2lb       ; Slot target count = Slot internal counter ?
     719A 830C 
     719C 830D 
0089 719E 1608  14         jne   tmgr10                ; No, get next slot
0090 71A0 0246  22         andi  tmp2,>ff00            ; Clear internal counter
     71A2 FF00 
0091 71A4 C506  30         mov   tmp2,*tmp0            ; Update timer table
0092               *--------------------------------------------------------------
0093               *  Run slot, we only need TMP0 to survive
0094               *--------------------------------------------------------------
0095 71A6 C804  38         mov   tmp0,@wtitmp          ; Save TMP0
     71A8 8330 
0096 71AA 0697  24         bl    *tmp3                 ; Call routine in slot
0097 71AC C120  34 slotok  mov   @wtitmp,tmp0          ; Restore TMP0
     71AE 8330 
0098               *--------------------------------------------------------------
0099               *  Prepare for next slot
0100               *--------------------------------------------------------------
0101 71B0 058A  14 tmgr10  inc   r10                   ; Next slot
0102 71B2 9820  54         cb    @r10lb,@btihi         ; Last slot done ?
     71B4 8315 
     71B6 8314 
0103 71B8 1504  14         jgt   tmgr12                ; yes, Wait for next VDP interrupt
0104 71BA 05C4  14         inct  tmp0                  ; Offset for next slot
0105 71BC 10E8  14         jmp   tmgr9                 ; Process next slot
0106 71BE 05C4  14 tmgr11  inct  tmp0                  ; Skip 2nd word of slot data
0107 71C0 10F7  14         jmp   tmgr10                ; Process next slot
0108 71C2 024A  22 tmgr12  andi  r10,>ff00             ; Use R10LB as tick counter. Reset.
     71C4 FF00 
0109 71C6 10B4  14         jmp   tmgr1
0110 71C8 0280     tmdat   data  >0280                 ; Bit 8 (kernel thread) and bit 6 (user hook)
0111               
**** **** ****     > runlib.asm
...
0014               ********@*****@*********************@**************************
0015 71CA E0A0  34 kthread soc   @wbit8,config         ; Block kernel thread
     71CC 603A 
0016               *--------------------------------------------------------------
...
0028               *--------------------------------------------------------------
0029               kthread_kb
0033 71CE 06A0  32         bl    @virtkb               ; Scan virtual keyboard
     71D0 63E2 
0035               *--------------------------------------------------------------
0036               * Scan real keyboard
0037               *--------------------------------------------------------------
0041 71D2 06A0  32         bl    @realkb               ; Scan full keyboard
     71D4 64D2 
0043               *--------------------------------------------------------------
0044               kthread_exit
0045 71D6 0460  28         b     @tmgr3                ; Exit
     71D8 7154 
**** **** ****     > runlib.asm
...
0016               ********@*****@*********************@**************************
0017 71DA C83B  50 mkhook  mov   *r11+,@wtiusr         ; Set user hook address
     71DC 832E 
0018 71DE E0A0  34         soc   @wbit7,config         ; Enable user hook
     71E0 603C 
0019 71E2 045B  20 mkhoo1  b     *r11                  ; Return
0020      7130     hookok  equ   tmgr1                 ; Exit point for user hook
0021               
0022               
0023               ***************************************************************
...
0026               *  BL    @CLHOOK
0027               ********@*****@*********************@**************************
0028 71E4 04E0  34 clhook  clr   @wtiusr               ; Unset user hook address
     71E6 832E 
0029 71E8 0242  22         andi  config,>feff          ; Disable user hook (bit 7=0)
     71EA FEFF 
0030 71EC 045B  20         b     *r11                  ; Return
**** **** ****     > runlib.asm
0211               
0215               
0216               
...
0234               ********@*****@*********************@**************************
0236 71EE 06A0  32 runlib  bl    @mem.scrpad.backup    ; Backup scratchpad memory to @>2000
     71F0 696A 
0237 71F2 04E0  34         clr   @>8302                ; Reset exit flag (R1 in workspace WS1!)
     71F4 8302 
0241               *--------------------------------------------------------------
0242               * Alternative entry point
0243               *--------------------------------------------------------------
0244 71F6 0300  24 runli1  limi  0                     ; Turn off interrupts
     71F8 0000 
0245 71FA 02E0  18         lwpi  ws1                   ; Activate workspace 1
     71FC 8300 
0246 71FE C0E0  34         mov   @>83c0,r3             ; Get random seed from OS monitor
     7200 83C0 
0247               *--------------------------------------------------------------
0248               * Clear scratch-pad memory from R4 upwards
0249               *--------------------------------------------------------------
0250 7202 0202  20 runli2  li    r2,>8308
     7204 8308 
0251 7206 04F2  30 runli3  clr   *r2+                  ; Clear scratchpad >8306->83FF
0252 7208 0282  22         ci    r2,>8400
     720A 8400 
0253 720C 16FC  14         jne   runli3
0254               *--------------------------------------------------------------
0255               * Exit to TI-99/4A title screen ?
0256               *--------------------------------------------------------------
0257               runli3a
0258 720E 0281  22         ci    r1,>ffff              ; Exit flag set ?
     7210 FFFF 
0259 7212 1602  14         jne   runli4                ; No, continue
0260 7214 0420  54         blwp  @0                    ; Yes, bye bye
     7216 0000 
0261               *--------------------------------------------------------------
0262               * Determine if VDP is PAL or NTSC
0263               *--------------------------------------------------------------
0264 7218 C803  38 runli4  mov   r3,@waux1             ; Store random seed
     721A 833C 
0265 721C 04C1  14         clr   r1                    ; Reset counter
0266 721E 0202  20         li    r2,10                 ; We test 10 times
     7220 000A 
0267 7222 C0E0  34 runli5  mov   @vdps,r3
     7224 8802 
0268 7226 20E0  38         coc   @wbit0,r3             ; Interupt flag set ?
     7228 604A 
0269 722A 1302  14         jeq   runli6
0270 722C 0581  14         inc   r1                    ; Increase counter
0271 722E 10F9  14         jmp   runli5
0272 7230 0602  14 runli6  dec   r2                    ; Next test
0273 7232 16F7  14         jne   runli5
0274 7234 0281  22         ci    r1,>1250              ; Max for NTSC reached ?
     7236 1250 
0275 7238 1202  14         jle   runli7                ; No, so it must be NTSC
0276 723A 0262  22         ori   config,palon          ; Yes, it must be PAL, set flag
     723C 6046 
0277               *--------------------------------------------------------------
0278               * Copy machine code to scratchpad (prepare tight loop)
0279               *--------------------------------------------------------------
0280 723E 0201  20 runli7  li    r1,mccode             ; Machinecode to patch
     7240 60A2 
0281 7242 0202  20         li    r2,mcloop+2           ; Scratch-pad reserved for machine code
     7244 8322 
0282 7246 CCB1  46         mov   *r1+,*r2+             ; Copy 1st instruction
0283 7248 CCB1  46         mov   *r1+,*r2+             ; Copy 2nd instruction
0284 724A CCB1  46         mov   *r1+,*r2+             ; Copy 3rd instruction
0285               *--------------------------------------------------------------
0286               * Initialize registers, memory, ...
0287               *--------------------------------------------------------------
0288 724C 04C1  14 runli9  clr   r1
0289 724E 04C2  14         clr   r2
0290 7250 04C3  14         clr   r3
0291 7252 0209  20         li    stack,>8400           ; Set stack
     7254 8400 
0292 7256 020F  20         li    r15,vdpw              ; Set VDP write address
     7258 8C00 
0296               *--------------------------------------------------------------
0297               * Setup video memory
0298               *--------------------------------------------------------------
0300 725A 0280  22         ci    r0,>4a4a              ; Crash flag set?
     725C 4A4A 
0301 725E 1605  14         jne   runlia
0302 7260 06A0  32         bl    @filv                 ; Clear 12K VDP memory instead
     7262 60DC 
0303 7264 0000             data  >0000,>00,>3fff       ; of 16K, so that PABs survive
     7266 0000 
     7268 3FFF 
0308 726A 06A0  32 runlia  bl    @filv
     726C 60DC 
0309 726E 0FC0             data  pctadr,spfclr,16      ; Load color table
     7270 00C1 
     7272 0010 
0310               *--------------------------------------------------------------
0311               * Check if there is a F18A present
0312               *--------------------------------------------------------------
0316 7274 06A0  32         bl    @f18unl               ; Unlock the F18A
     7276 6352 
0317 7278 06A0  32         bl    @f18chk               ; Check if F18A is there
     727A 636C 
0318 727C 06A0  32         bl    @f18lck               ; Lock the F18A again
     727E 6362 
0320               *--------------------------------------------------------------
...
0330               *--------------------------------------------------------------
0331 7280 06A0  32 runlic  bl    @vidtab               ; Load video mode table into VDP
     7282 6136 
0332 7284 608E             data  spvmod                ; Equate selected video mode table
0333 7286 0204  20         li    tmp0,spfont           ; Get font option
     7288 000C 
0334 728A 0544  14         inv   tmp0                  ; NOFONT (>FFFF) specified ?
0335 728C 1304  14         jeq   runlid                ; Yes, skip it
0336 728E 06A0  32         bl    @ldfnt
     7290 619E 
0337 7292 1100             data  fntadr,spfont         ; Load specified font
     7294 000C 
0338               *--------------------------------------------------------------
0339               * Did a system crash occur before runlib was called?
0340               *--------------------------------------------------------------
0341 7296 0280  22 runlid  ci    r0,>4a4a              ; Crash flag set?
     7298 4A4A 
0342 729A 1602  14         jne   runlie                ; No, continue
0343 729C 0460  28         b     @crash_handler.main   ; Yes, back to crash handler
     729E 6060 
0344               *--------------------------------------------------------------
0345               * Branch to main program
0346               *--------------------------------------------------------------
0347 72A0 0262  22 runlie  ori   config,>0040          ; Enable kernel thread (bit 9 on)
     72A2 0040 
0348 72A4 0460  28         b     @main                 ; Give control to main program
     72A6 72A8 
**** **** ****     > fio.asm.30488
0071               *--------------------------------------------------------------
...
0078               ;--------------------------------------------------------------
0079      608E     spvmod  equ   tx8024                ; Video mode.   See VIDTAB for details.
...
0085               ;--------------------------------------------------------------
0086      01F0     pabadr1 equ   >01f0                 ; VDP PAB1
0087      0200     pabadr2 equ   >0200                 ; VDP PAB2
0088      0300     vrecbuf equ   >0300                 ; VDP Buffer
0089               
0090               ***************************************************************
0091               * Main
0092               ********@*****@*********************@**************************
0093 72A8 06A0  32 main    bl    @putat
     72AA 626E 
0094 72AC 0000             data  >0000,msg
     72AE 731E 
0095               
0096 72B0 06A0  32         bl    @putat
     72B2 626E 
0097 72B4 0100             data  >0100,fname
     72B6 730D 
0098               
0099                       ;------------------------------------------------------
0100                       ; Prepare VDP for PAB and page out scratchpad
0101                       ;------------------------------------------------------
0102 72B8 06A0  32         bl    @cpym2v
     72BA 6276 
0103 72BC 01F0             data  pabadr1,dsrsub,2      ; Copy PAB for DSR call files subprogram
     72BE 7302 
     72C0 0002 
0104               
0105 72C2 06A0  32         bl    @cpym2v
     72C4 6276 
0106 72C6 0200             data  pabadr2,pab,25        ; Copy PAB to VDP
     72C8 7304 
     72CA 0019 
0107               
0108 72CC 06A0  32         bl    @mem.scrpad.pgout     ; Page out scratchpad memory
     72CE 6F6E 
0109 72D0 A000                   data  >a000           ; Memory destination @>a000
0110               
0111                       ;------------------------------------------------------
0112                       ; Open file
0113                       ;------------------------------------------------------
0114 72D2 06A0  32         bl    @file.open
     72D4 70A6 
0115 72D6 0200             data  pabadr2                ; Pass file descriptor to DSRLNK
0116               
0117                       ;------------------------------------------------------
0118                       ; Read record
0119                       ;------------------------------------------------------
0120               readfile
0121 72D8 06A0  32         bl    @file.record.read
     72DA 70E4 
0122 72DC 0200             data  pabadr2
0123               
0124 72DE 130F  14         jeq   file_error
0125 72E0 10FB  14         jmp   readfile
0126               
0127                       ;------------------------------------------------------
0128                       ; Close file
0129                       ;------------------------------------------------------
0130               close_file
0131 72E2 0200  20         li    r0,pabadr2+9
     72E4 0209 
0132 72E6 C800  38         mov   r0,@>8356             ; Pass file descriptor to DSRLNK
     72E8 8356 
0133               
0134 72EA 06A0  32         bl    @vputb
     72EC 6116 
0135 72EE 0200             data  pabadr2,io.op.close
     72F0 0001 
0136               
0137 72F2 0420  54         blwp  @dsrlnk
     72F4 6F90 
0138 72F6 0008             data  8
0139               
0140 72F8 10FF  14 done0   jmp   $
0141 72FA 10FF  14 done1   jmp   $
0142 72FC 10FF  14 done2   jmp   $
0143               
0144               file_error
0145 72FE 10F1  14         jmp   close_file
0146               
0147               
0148               
0149               
0150               
0151 7300 10FF  14 haltme  jmp   $
0152               
0153               
0154               
0155               ***************************************************************
0156               * DSR subprogram for call files
0157               ***************************************************************
0158                       even
0159 7302 0116     dsrsub  byte  >01,>16               ; DSR program/subprogram - set file buffers
0160               
0161               
0162               ***************************************************************
0163               * PAB for accessing file
0164               ********@*****@*********************@**************************
0165 7304 0014     pab     byte  io.op.open            ;  0    - OPEN
0166                       byte  io.ft.sf.ivd          ;  1    - INPUT, VARIABLE, DISPLAY
0167 7306 0300             data  vrecbuf               ;  2-3  - Record buffer in VDP memory
0168 7308 5000             byte  80                    ;  4    - Record length (80 characters maximum)
0169                       byte  00                    ;  5    - Character count
0170 730A 0000             data  >0000                 ;  6-7  - Seek record (only for fixed records)
0171 730C 000F             byte  >00                   ;  8    - Screen offset (cassette DSR only)
0172               fname   byte  15                    ;  9    - File descriptor length
0173 730E ....             text 'DSK1.SPEECHDOCS'      ; 10-.. - File descriptor (Device + '.' + File name)
0174                       even
0175               
0176               
0177               msg
0178 731E 152A             byte  21
0179 731F ....             text  '* File reading test *'
0180                       even
0181               
0182 7334 00AA     schrott data  >00aa, >3fff, >1103
     7336 3FFF 
     7338 1103 
0183               
0184               
