...
0006               *
0007               ***************************************************************
0008               * File: fio.asm                     ; Version 191117-2708
0009               *--------------------------------------------------------------
...
0053 6012 6FAC             data  runlib
0055               
0056 6014 1046             byte  16
0057 6015 ....             text  'FIOT 191117-2708'
0058                       even
0059               
0067               *--------------------------------------------------------------
...
0031 6D40 0206  20         li    tmp2,128              ; tmp2 = Bytes to copy
     6D42 0080 
0032                       ;------------------------------------------------------
0033                       ; Copy memory
0034                       ;------------------------------------------------------
...
0040               ********@*****@*********************@**************************
0041 6D5A B000     dsrlnk  data  dsrlnk.dsrlws         ; dsrlnk workspace
0042 6D5C 6D5E             data  dsrlnk.init           ; entry point
0043                       ;------------------------------------------------------
0044                       ; DSRLNK entry point
0045                       ;------------------------------------------------------
0046               dsrlnk.init:
0047 6D5E C17E  30         mov   *r14+,r5              ; get pgm type for link
0048 6D60 C805  38         mov   r5,@dsrlnk.sav8a      ; save data following blwp @dsrlnk (8 or >a)
     6D62 8322 
0049 6D64 53E0  34         szcb  @hb$20,r15            ; reset equal bit
     6D66 6042 
0050 6D68 C020  34         mov   @>8356,r0             ; get ptr to pab
     6D6A 8356 
0051 6D6C C240  18         mov   r0,r9                 ; save ptr
0052                       ;------------------------------------------------------
0053                       ; Fetch file descriptor length from PAB
0054                       ;------------------------------------------------------
0055 6D6E 0229  22         ai    r9,>fff8              ; adjust r9 to addr PAB flag -> (pabaddr+9)-8
     6D70 FFF8 
0056               
0057                       ;---------------------------; Inline VSBR start
0058 6D72 06C0  14         swpb  r0                    ;
0059 6D74 D800  38         movb  r0,@vdpa              ; send low byte
     6D76 8C02 
0060 6D78 06C0  14         swpb  r0                    ;
0061 6D7A D800  38         movb  r0,@vdpa              ; send high byte
     6D7C 8C02 
0062 6D7E D0E0  34         movb  @vdpr,r3              ; read byte from VDP ram
     6D80 8800 
0063                       ;---------------------------; Inline VSBR end
0064 6D82 0983  56         srl   r3,8                  ; Move to low byte
0065               
0066                       ;------------------------------------------------------
0067                       ; Fetch file descriptor device name from PAB
0068                       ;------------------------------------------------------
0069 6D84 0704  14         seto  r4                    ; init counter
0070 6D86 0202  20         li    r2,dsrlnk.namsto      ; point to 8-byte CPU buffer
     6D88 2100 
0071 6D8A 0580  14 !       inc   r0                    ; point to next char of name
0072 6D8C 0584  14         inc   r4                    ; incr char counter
0073 6D8E 0284  22         ci    r4,>0007              ; see if length more than 7 chars
     6D90 0007 
0074 6D92 1565  14         jgt   dsrlnk.error.devicename_invalid
0075                                                   ; yes, error
0076 6D94 80C4  18         c     r4,r3                 ; end of name?
0077 6D96 130C  14         jeq   dsrlnk.device_name.get_length
0078                                                   ; yes
0079               
0080                       ;---------------------------; Inline VSBR start
0081 6D98 06C0  14         swpb  r0                    ;
0082 6D9A D800  38         movb  r0,@vdpa              ; send low byte
     6D9C 8C02 
0083 6D9E 06C0  14         swpb  r0                    ;
0084 6DA0 D800  38         movb  r0,@vdpa              ; send high byte
     6DA2 8C02 
0085 6DA4 D060  34         movb  @vdpr,r1              ; read byte from VDP ram
     6DA6 8800 
...
0090                       ;------------------------------------------------------
0091 6DA8 DC81  32         movb  r1,*r2+               ; move into buffer
0092 6DAA 9801  38         cb    r1,@dsrlnk.period     ; is character a '.'
     6DAC 6E6E 
0093 6DAE 16ED  14         jne   -!                    ; no, loop next char
0094                       ;------------------------------------------------------
0095                       ; Determine device name length
0096                       ;------------------------------------------------------
0097               dsrlnk.device_name.get_length:
0098 6DB0 C104  18         mov   r4,r4                 ; Check if length = 0
0099 6DB2 1355  14         jeq   dsrlnk.error.devicename_invalid
0100                                                   ; yes, error
0101 6DB4 04E0  34         clr   @>83d0
     6DB6 83D0 
0102 6DB8 C804  38         mov   r4,@>8354             ; save name length for search
     6DBA 8354 
0103 6DBC 0584  14         inc   r4                    ; adjust for dot
0104 6DBE A804  38         a     r4,@>8356             ; point to position after name
     6DC0 8356 
0105                       ;------------------------------------------------------
0106                       ; Prepare for DSR scan >1000 - >1f00
0107                       ;------------------------------------------------------
0108               dsrlnk.dsrscan.start:
0109 6DC2 02E0  18         lwpi  >83e0                 ; Use GPL WS
     6DC4 83E0 
0110 6DC6 04C1  14         clr   r1                    ; version found of dsr
0111 6DC8 020C  20         li    r12,>0f00             ; init cru addr
     6DCA 0F00 
0112                       ;------------------------------------------------------
0113                       ; Turn off ROM on current card
0114                       ;------------------------------------------------------
0115               dsrlnk.dsrscan.cardoff:
0116 6DCC C30C  18         mov   r12,r12               ; anything to turn off?
0117 6DCE 1301  14         jeq   dsrlnk.dsrscan.cardloop
0118                                                   ; no, loop over cards
0119 6DD0 1E00  20         sbz   0                     ; yes, turn off
0120                       ;------------------------------------------------------
0121                       ; Loop over cards and look if DSR present
0122                       ;------------------------------------------------------
0123               dsrlnk.dsrscan.cardloop:
0124 6DD2 022C  22         ai    r12,>0100             ; next rom to turn on
     6DD4 0100 
0125 6DD6 04E0  34         clr   @>83d0                ; clear in case we are done
     6DD8 83D0 
0126 6DDA 028C  22         ci    r12,>2000             ; Card scan complete? (>1000 to >1F00)
     6DDC 2000 
0127 6DDE 133D  14         jeq   dsrlnk.error.nodsr_found
0128                                                   ; yes, no matching DSR found
0129 6DE0 C80C  38         mov   r12,@>83d0            ; save addr of next cru
     6DE2 83D0 
0130                       ;------------------------------------------------------
0131                       ; Look at card ROM (@>4000 eq 'AA' ?)
0132                       ;------------------------------------------------------
0133 6DE4 1D00  20         sbo   0                     ; turn on rom
0134 6DE6 0202  20         li    r2,>4000              ; start at beginning of rom
     6DE8 4000 
0135 6DEA 9812  46         cb    *r2,@dsrlnk.$aa00     ; check for a valid DSR header
     6DEC 6E6A 
0136 6DEE 16EE  14         jne   dsrlnk.dsrscan.cardoff
...
0144                       ;------------------------------------------------------
0145 6DF0 A0A0  34         a     @dsrlnk.dstype,r2     ; go to first pointer (byte 8 or 10)
     6DF2 B00A 
0146 6DF4 1003  14         jmp   dsrlnk.dsrscan.getentry
0147                       ;------------------------------------------------------
0148                       ; Next DSR entry
0149                       ;------------------------------------------------------
0150               dsrlnk.dsrscan.nextentry:
0151 6DF6 C0A0  34         mov   @>83d2,r2             ; Offset 0 > Fetch link to next DSR or subprogram
     6DF8 83D2 
0152 6DFA 1D00  20         sbo   0                     ; turn rom back on
0153                       ;------------------------------------------------------
0154                       ; Get DSR entry
0155                       ;------------------------------------------------------
0156               dsrlnk.dsrscan.getentry:
0157 6DFC C092  26         mov   *r2,r2                ; is addr a zero? (end of chain?)
0158 6DFE 13E6  14         jeq   dsrlnk.dsrscan.cardoff
0159                                                   ; yes, no more DSRs or programs to check
0160 6E00 C802  38         mov   r2,@>83d2             ; Offset 0 > Store link to next DSR or subprogram
     6E02 83D2 
0161 6E04 05C2  14         inct  r2                    ; Offset 2 > Has call address of current DSR/subprogram code
0162 6E06 C272  30         mov   *r2+,r9               ; Store call address in r9. Move r2 to offset 4 (DSR/subprogram name)
0163                       ;------------------------------------------------------
0164                       ; Check file descriptor in DSR
0165                       ;------------------------------------------------------
0166 6E08 04C5  14         clr   r5                    ; Remove any old stuff
0167 6E0A D160  34         movb  @>8355,r5             ; get length as counter
     6E0C 8355 
0168 6E0E 130B  14         jeq   dsrlnk.dsrscan.call_dsr
0169                                                   ; if zero, do not further check, call DSR program
0170 6E10 9C85  32         cb    r5,*r2+               ; see if length matches
0171 6E12 16F1  14         jne   dsrlnk.dsrscan.nextentry
0172                                                   ; no, length does not match. Go process next DSR entry
0173 6E14 0985  56         srl   r5,8                  ; yes, move to low byte
0174 6E16 0206  20         li    r6,dsrlnk.namsto      ; Point to 8-byte CPU buffer
     6E18 2100 
0175 6E1A 9CB6  42 !       cb    *r6+,*r2+             ; compare byte in CPU buffer with byte in DSR ROM
0176 6E1C 16EC  14         jne   dsrlnk.dsrscan.nextentry
0177                                                   ; try next DSR entry if no match
0178 6E1E 0605  14         dec   r5                    ; loop until full length checked
0179 6E20 16FC  14         jne   -!
0180                       ;------------------------------------------------------
0181                       ; Device name/Subprogram match
0182                       ;------------------------------------------------------
0183               dsrlnk.dsrscan.match:
0184 6E22 C802  38         mov   r2,@>83d2             ; DSR entry addr must be saved at @>83d2
     6E24 83D2 
0185               
0186                       ;------------------------------------------------------
...
0190 6E26 0581  14         inc   r1                    ; next version found
0191 6E28 0699  24         bl    *r9                   ; go run routine
0192                       ;
0193                       ; Depending on IO result the DSR in card ROM does RET
0194                       ; or (INCT R11 + RET), meaning either (1) or (2) get executed.
0195                       ;
0196 6E2A 10E5  14         jmp   dsrlnk.dsrscan.nextentry
0197                                                   ; (1) error return
0198 6E2C 1E00  20         sbz   0                     ; (2) turn off rom if good return
0199 6E2E 02E0  18         lwpi  dsrlnk.dsrlws         ; (2) restore workspace
     6E30 B000 
0200 6E32 C009  18         mov   r9,r0                 ; point to flag in pab
0201 6E34 C060  34         mov   @dsrlnk.sav8a,r1      ; get back data following blwp @dsrlnk
     6E36 8322 
0202                                                   ; (8 or >a)
0203 6E38 0281  22         ci    r1,8                  ; was it 8?
     6E3A 0008 
0204 6E3C 1303  14         jeq   dsrlnk.dsrscan.dsr.8  ; yes, jump: normal dsrlnk
0205 6E3E D060  34         movb  @>8350,r1             ; no, we have a data >a.
     6E40 8350 
0206                                                   ; Get error byte from @>8350
0207 6E42 1008  14         jmp   dsrlnk.dsrscan.dsr.a  ; go and return error byte to the caller
0208               
0209                       ;------------------------------------------------------
...
0214 6E44 06C0  14         swpb  r0                    ;
0215 6E46 D800  38         movb  r0,@vdpa              ; send low byte
     6E48 8C02 
0216 6E4A 06C0  14         swpb  r0                    ;
0217 6E4C D800  38         movb  r0,@vdpa              ; send high byte
     6E4E 8C02 
0218 6E50 D060  34         movb  @vdpr,r1              ; read byte from VDP ram
     6E52 8800 
...
0225 6E54 09D1  56         srl   r1,13                 ; just keep error bits
0226 6E56 1604  14         jne   dsrlnk.error.io_error
0227                                                   ; handle IO error
0228 6E58 0380  18         rtwp                        ; Return from DSR workspace to caller workspace
0229               
0230                       ;------------------------------------------------------
0231                       ; IO-error handler
0232                       ;------------------------------------------------------
0233               dsrlnk.error.nodsr_found:
0234 6E5A 02E0  18         lwpi  dsrlnk.dsrlws         ; No DSR found, restore workspace
     6E5C B000 
0235               dsrlnk.error.devicename_invalid:
0236 6E5E 04C1  14         clr   r1                    ; clear flag for error 0 = bad device name
0237               dsrlnk.error.io_error:
0238 6E60 06C1  14         swpb  r1                    ; put error in hi byte
0239 6E62 D741  30         movb  r1,*r13               ; store error flags in callers r0
0240 6E64 F3E0  34         socb  @hb$20,r15            ; set equal bit to indicate error
     6E66 6042 
0241 6E68 0380  18         rtwp                        ; Return from DSR workspace to caller workspace
0242               
0243               ****************************************************************************************
0244               
0245 6E6A AA00     dsrlnk.$aa00      data   >aa00      ; Used for identifying DSR header
0246 6E6C 0008     dsrlnk.$0008      data   >0008      ; 8 is the data that usually follows a @blwp @dsrlnk
0247 6E6E ....     dsrlnk.period     text  '.'         ; For finding end of device name
0248               
0249                       even
**** **** ****     > runlib.asm
...
0124               ********@*****@*********************@**************************
0125               file.open:
0126 6E70 C03B  30         mov   *r11+,r0              ; Get file descriptor (P0)
0127               *--------------------------------------------------------------
0128               * Initialisation
0129               *--------------------------------------------------------------
0130               xfile.open:
0131 6E72 C04B  18         mov   r11,r1                ; Save return address
0132 6E74 C100  18         mov   r0,tmp0               ; VDP write address (PAB byte 0)
0133 6E76 04C5  14         clr   tmp1                  ; io.op.open
0134 6E78 06A0  32         bl    @xvputb               ; Write file opcode to VDP
     6E7A 6116 
0135               file.open_init:
0136 6E7C 0220  22         ai    r0,9                  ; Move to file descriptor length
     6E7E 0009 
0137 6E80 C800  38         mov   r0,@>8356             ; Pass file descriptor to DSRLNK
     6E82 8356 
0138               *--------------------------------------------------------------
0139               * Main
0140               *--------------------------------------------------------------
0141               file.open_main:
0142 6E84 0420  54         blwp  @dsrlnk               ; Call DSRLNK
     6E86 6D5A 
0143 6E88 0008             data  8                     ; Level 2 IO call
0144               *--------------------------------------------------------------
0145               * Exit
0146               *--------------------------------------------------------------
0147               file.open_exit:
0148 6E8A 1025  14         jmp   file.record.pab.details
...
0170               ********@*****@*********************@**************************
0171               file.close:
0172 6E8C C03B  30         mov   *r11+,r0              ; Get file descriptor (P0)
0173               *--------------------------------------------------------------
0174               * Initialisation
0175               *--------------------------------------------------------------
0176               xfile.close:
0177 6E8E C04B  18         mov   r11,r1                ; Save return address
0178 6E90 C100  18         mov   r0,tmp0               ; VDP write address (PAB byte 0)
0179 6E92 0205  20         li    tmp1,io.op.close      ; io.op.close
     6E94 0001 
0180 6E96 06A0  32         bl    @xvputb               ; Write file opcode to VDP
     6E98 6116 
0181               file.close_init:
0182 6E9A 0220  22         ai    r0,9                  ; Move to file descriptor length
     6E9C 0009 
0183 6E9E C800  38         mov   r0,@>8356             ; Pass file descriptor to DSRLNK
     6EA0 8356 
0184               *--------------------------------------------------------------
0185               * Main
0186               *--------------------------------------------------------------
0187               file.close_main:
0188 6EA2 0420  54         blwp  @dsrlnk               ; Call DSRLNK
     6EA4 6D5A 
0189 6EA6 0008             data  8                     ;
0190               *--------------------------------------------------------------
0191               * Exit
0192               *--------------------------------------------------------------
0193               file.close_exit:
0194 6EA8 1016  14         jmp   file.record.pab.details
...
0220 6EAA C03B  30         mov   *r11+,r0              ; Get file descriptor (P0)
0221               *--------------------------------------------------------------
0222               * Initialisation
0223               *--------------------------------------------------------------
0224               xfile.record.read:
0225 6EAC C04B  18         mov   r11,r1                ; Save return address
0226 6EAE C100  18         mov   r0,tmp0               ; VDP write address (PAB byte 0)
0227 6EB0 0205  20         li    tmp1,io.op.read       ; io.op.read
     6EB2 0002 
0228 6EB4 06A0  32         bl    @xvputb               ; Write file opcode to VDP
     6EB6 6116 
0229               file.record.read_init:
0230 6EB8 0220  22         ai    r0,9                  ; Move to file descriptor length
     6EBA 0009 
0231 6EBC C800  38         mov   r0,@>8356             ; Pass file descriptor to DSRLNK
     6EBE 8356 
0232               *--------------------------------------------------------------
0233               * Main
0234               *--------------------------------------------------------------
0235               file.record.read_main:
0236 6EC0 0420  54         blwp  @dsrlnk               ; Call DSRLNK
     6EC2 6D5A 
0237 6EC4 0008             data  8                     ;
0238               *--------------------------------------------------------------
0239               * Exit
0240               *--------------------------------------------------------------
0241               file.record.read_exit:
0242 6EC6 1007  14         jmp   file.record.pab.details
...
0250 6EC8 1000  14         nop
0251               
0252               
0253               file.record.seek:
0254 6ECA 1000  14         nop
0255               
0256               
0257               file.image.load:
0258 6ECC 1000  14         nop
0259               
0260               
0261               file.image.save:
0262 6ECE 1000  14         nop
0263               
0264               
0265               file.delete:
0266 6ED0 1000  14         nop
0267               
0268               
0269               file.rename:
0270 6ED2 1000  14         nop
0271               
0272               
0273               file.status:
0274 6ED4 1000  14         nop
0275               
0276               
0277               
0278               ***************************************************************
...
0291 6ED6 02C6  12         stst  tmp2                  ; Store status register contents in tmp2
...
0297               *--------------------------------------------------------------
0298 6ED8 C120  34         mov   @>8356,tmp0           ; Get PAB VDP address + 9
     6EDA 8356 
0299 6EDC 0224  22         ai    tmp0,-4               ; Get address of PAB + 5
     6EDE FFFC 
0300 6EE0 06A0  32         bl    @xvgetb               ; VDP read PAB status byte into tmp0
     6EE2 612E 
0301 6EE4 C144  18         mov   tmp0,tmp1             ; Move to destination
0302               *--------------------------------------------------------------
0303               * Get PAB byte 1 from VDP ram into tmp0 (status)
0304               *--------------------------------------------------------------
0305 6EE6 C100  18         mov   r0,tmp0               ; VDP PAB byte 1 (status)
...
0329 6EE8 0451  20         b     *r1                   ; Return to caller
**** **** ****     > runlib.asm
0201               
0202               
0203               
...
0019               ********@*****@*********************@**************************
0020 6EEA 0300  24 tmgr    limi  0                     ; No interrupt processing
     6EEC 0000 
0021               *--------------------------------------------------------------
0022               * Read VDP status register
0023               *--------------------------------------------------------------
0024 6EEE D360  34 tmgr1   movb  @vdps,r13             ; Save copy of VDP status register in R13
     6EF0 8802 
0025               *--------------------------------------------------------------
0026               * Latch sprite collision flag
0027               *--------------------------------------------------------------
0028 6EF2 2360  38         coc   @wbit2,r13            ; C flag on ?
     6EF4 6042 
0029 6EF6 1602  14         jne   tmgr1a                ; No, so move on
0030 6EF8 E0A0  34         soc   @wbit12,config        ; Latch bit 12 in config register
     6EFA 602E 
0031               *--------------------------------------------------------------
0032               * Interrupt flag
0033               *--------------------------------------------------------------
0034 6EFC 2360  38 tmgr1a  coc   @wbit0,r13            ; Interupt flag set ?
     6EFE 6046 
0035 6F00 1311  14         jeq   tmgr4                 ; Yes, process slots 0..n
0036               *--------------------------------------------------------------
...
0046               *--------------------------------------------------------------
0047 6F02 20A0  38 tmgr2   coc   @wbit8,config         ; Kernel thread blocked ?
     6F04 6036 
0048 6F06 1305  14         jeq   tmgr3                 ; Yes, skip to user hook
0049 6F08 20A0  38         coc   @wbit9,config         ; Kernel thread enabled ?
     6F0A 6034 
0050 6F0C 1602  14         jne   tmgr3                 ; No, skip to user hook
0051 6F0E 0460  28         b     @kthread              ; Run kernel thread
     6F10 6F88 
0052               *--------------------------------------------------------------
0053               * Run user hook
0054               *--------------------------------------------------------------
0055 6F12 20A0  38 tmgr3   coc   @wbit6,config         ; User hook blocked ?
     6F14 603A 
0056 6F16 13EB  14         jeq   tmgr1
0057 6F18 20A0  38         coc   @wbit7,config         ; User hook enabled ?
     6F1A 6038 
0058 6F1C 16E8  14         jne   tmgr1
0059 6F1E C120  34         mov   @wtiusr,tmp0
     6F20 832E 
0060 6F22 0454  20         b     *tmp0                 ; Run user hook
0061               *--------------------------------------------------------------
0062               * Do internal housekeeping
0063               *--------------------------------------------------------------
0064 6F24 40A0  34 tmgr4   szc   @tmdat,config         ; Unblock kernel thread and user hook
     6F26 6F86 
0065 6F28 C10A  18         mov   r10,tmp0
0066 6F2A 0244  22         andi  tmp0,>00ff            ; Clear HI byte
     6F2C 00FF 
0067 6F2E 20A0  38         coc   @wbit2,config         ; PAL flag set ?
     6F30 6042 
0068 6F32 1303  14         jeq   tmgr5
0069 6F34 0284  22         ci    tmp0,60               ; 1 second reached ?
     6F36 003C 
0070 6F38 1002  14         jmp   tmgr6
0071 6F3A 0284  22 tmgr5   ci    tmp0,50
     6F3C 0032 
0072 6F3E 1101  14 tmgr6   jlt   tmgr7                 ; No, continue
0073 6F40 1001  14         jmp   tmgr8
0074 6F42 058A  14 tmgr7   inc   r10                   ; Increase tick counter
0075               *--------------------------------------------------------------
0076               * Loop over slots
0077               *--------------------------------------------------------------
0078 6F44 C120  34 tmgr8   mov   @wtitab,tmp0          ; Pointer to timer table
     6F46 832C 
0079 6F48 024A  22         andi  r10,>ff00             ; Use R10LB as slot counter. Reset.
     6F4A FF00 
0080 6F4C C1D4  26 tmgr9   mov   *tmp0,tmp3            ; Is slot empty ?
0081 6F4E 1316  14         jeq   tmgr11                ; Yes, get next slot
0082               *--------------------------------------------------------------
0083               *  Check if slot should be executed
0084               *--------------------------------------------------------------
0085 6F50 05C4  14         inct  tmp0                  ; Second word of slot data
0086 6F52 0594  26         inc   *tmp0                 ; Update tick count in slot
0087 6F54 C194  26         mov   *tmp0,tmp2            ; Get second word of slot data
0088 6F56 9820  54         cb    @tmp2hb,@tmp2lb       ; Slot target count = Slot internal counter ?
     6F58 830C 
     6F5A 830D 
0089 6F5C 1608  14         jne   tmgr10                ; No, get next slot
0090 6F5E 0246  22         andi  tmp2,>ff00            ; Clear internal counter
     6F60 FF00 
0091 6F62 C506  30         mov   tmp2,*tmp0            ; Update timer table
0092               *--------------------------------------------------------------
0093               *  Run slot, we only need TMP0 to survive
0094               *--------------------------------------------------------------
0095 6F64 C804  38         mov   tmp0,@wtitmp          ; Save TMP0
     6F66 8330 
0096 6F68 0697  24         bl    *tmp3                 ; Call routine in slot
0097 6F6A C120  34 slotok  mov   @wtitmp,tmp0          ; Restore TMP0
     6F6C 8330 
0098               *--------------------------------------------------------------
0099               *  Prepare for next slot
0100               *--------------------------------------------------------------
0101 6F6E 058A  14 tmgr10  inc   r10                   ; Next slot
0102 6F70 9820  54         cb    @r10lb,@btihi         ; Last slot done ?
     6F72 8315 
     6F74 8314 
0103 6F76 1504  14         jgt   tmgr12                ; yes, Wait for next VDP interrupt
0104 6F78 05C4  14         inct  tmp0                  ; Offset for next slot
0105 6F7A 10E8  14         jmp   tmgr9                 ; Process next slot
0106 6F7C 05C4  14 tmgr11  inct  tmp0                  ; Skip 2nd word of slot data
0107 6F7E 10F7  14         jmp   tmgr10                ; Process next slot
0108 6F80 024A  22 tmgr12  andi  r10,>ff00             ; Use R10LB as tick counter. Reset.
     6F82 FF00 
0109 6F84 10B4  14         jmp   tmgr1
0110 6F86 0280     tmdat   data  >0280                 ; Bit 8 (kernel thread) and bit 6 (user hook)
0111               
**** **** ****     > runlib.asm
...
0014               ********@*****@*********************@**************************
0015 6F88 E0A0  34 kthread soc   @wbit8,config         ; Block kernel thread
     6F8A 6036 
0016               *--------------------------------------------------------------
...
0028               *--------------------------------------------------------------
0029               kthread_kb
0033 6F8C 06A0  32         bl    @virtkb               ; Scan virtual keyboard
     6F8E 63EE 
0035               *--------------------------------------------------------------
0036               * Scan real keyboard
0037               *--------------------------------------------------------------
0041 6F90 06A0  32         bl    @realkb               ; Scan full keyboard
     6F92 64DE 
0043               *--------------------------------------------------------------
0044               kthread_exit
0045 6F94 0460  28         b     @tmgr3                ; Exit
     6F96 6F12 
**** **** ****     > runlib.asm
...
0016               ********@*****@*********************@**************************
0017 6F98 C83B  50 mkhook  mov   *r11+,@wtiusr         ; Set user hook address
     6F9A 832E 
0018 6F9C E0A0  34         soc   @wbit7,config         ; Enable user hook
     6F9E 6038 
0019 6FA0 045B  20 mkhoo1  b     *r11                  ; Return
0020      6EEE     hookok  equ   tmgr1                 ; Exit point for user hook
0021               
0022               
0023               ***************************************************************
...
0026               *  BL    @CLHOOK
0027               ********@*****@*********************@**************************
0028 6FA2 04E0  34 clhook  clr   @wtiusr               ; Unset user hook address
     6FA4 832E 
0029 6FA6 0242  22         andi  config,>feff          ; Disable user hook (bit 7=0)
     6FA8 FEFF 
0030 6FAA 045B  20         b     *r11                  ; Return
**** **** ****     > runlib.asm
0211               
0215               
0216               
...
0234               ********@*****@*********************@**************************
0236 6FAC 06A0  32 runlib  bl    @mem.scrpad.backup    ; Backup scratchpad memory to @>2000
     6FAE 6734 
0237 6FB0 04E0  34         clr   @>8302                ; Reset exit flag (R1 in workspace WS1!)
     6FB2 8302 
0241               *--------------------------------------------------------------
0242               * Alternative entry point
0243               *--------------------------------------------------------------
0244 6FB4 0300  24 runli1  limi  0                     ; Turn off interrupts
     6FB6 0000 
0245 6FB8 02E0  18         lwpi  ws1                   ; Activate workspace 1
     6FBA 8300 
0246 6FBC C0E0  34         mov   @>83c0,r3             ; Get random seed from OS monitor
     6FBE 83C0 
0247               *--------------------------------------------------------------
0248               * Clear scratch-pad memory from R4 upwards
0249               *--------------------------------------------------------------
0250 6FC0 0202  20 runli2  li    r2,>8308
     6FC2 8308 
0251 6FC4 04F2  30 runli3  clr   *r2+                  ; Clear scratchpad >8306->83FF
0252 6FC6 0282  22         ci    r2,>8400
     6FC8 8400 
0253 6FCA 16FC  14         jne   runli3
0254               *--------------------------------------------------------------
0255               * Exit to TI-99/4A title screen ?
0256               *--------------------------------------------------------------
0257               runli3a
0258 6FCC 0281  22         ci    r1,>ffff              ; Exit flag set ?
     6FCE FFFF 
0259 6FD0 1602  14         jne   runli4                ; No, continue
0260 6FD2 0420  54         blwp  @0                    ; Yes, bye bye
     6FD4 0000 
0261               *--------------------------------------------------------------
0262               * Determine if VDP is PAL or NTSC
0263               *--------------------------------------------------------------
0264 6FD6 C803  38 runli4  mov   r3,@waux1             ; Store random seed
     6FD8 833C 
0265 6FDA 04C1  14         clr   r1                    ; Reset counter
0266 6FDC 0202  20         li    r2,10                 ; We test 10 times
     6FDE 000A 
0267 6FE0 C0E0  34 runli5  mov   @vdps,r3
     6FE2 8802 
0268 6FE4 20E0  38         coc   @wbit0,r3             ; Interupt flag set ?
     6FE6 6046 
0269 6FE8 1302  14         jeq   runli6
0270 6FEA 0581  14         inc   r1                    ; Increase counter
0271 6FEC 10F9  14         jmp   runli5
0272 6FEE 0602  14 runli6  dec   r2                    ; Next test
0273 6FF0 16F7  14         jne   runli5
0274 6FF2 0281  22         ci    r1,>1250              ; Max for NTSC reached ?
     6FF4 1250 
0275 6FF6 1202  14         jle   runli7                ; No, so it must be NTSC
0276 6FF8 0262  22         ori   config,palon          ; Yes, it must be PAL, set flag
     6FFA 6042 
0277               *--------------------------------------------------------------
0278               * Copy machine code to scratchpad (prepare tight loop)
0279               *--------------------------------------------------------------
0280 6FFC 0201  20 runli7  li    r1,mccode             ; Machinecode to patch
     6FFE 609E 
0281 7000 0202  20         li    r2,mcloop+2           ; Scratch-pad reserved for machine code
     7002 8322 
0282 7004 CCB1  46         mov   *r1+,*r2+             ; Copy 1st instruction
0283 7006 CCB1  46         mov   *r1+,*r2+             ; Copy 2nd instruction
0284 7008 CCB1  46         mov   *r1+,*r2+             ; Copy 3rd instruction
0285               *--------------------------------------------------------------
0286               * Initialize registers, memory, ...
0287               *--------------------------------------------------------------
0288 700A 04C1  14 runli9  clr   r1
0289 700C 04C2  14         clr   r2
0290 700E 04C3  14         clr   r3
0291 7010 0209  20         li    stack,>8400           ; Set stack
     7012 8400 
0292 7014 020F  20         li    r15,vdpw              ; Set VDP write address
     7016 8C00 
0296               *--------------------------------------------------------------
0297               * Setup video memory
0298               *--------------------------------------------------------------
0300 7018 0280  22         ci    r0,>4a4a              ; Crash flag set?
     701A 4A4A 
0301 701C 1605  14         jne   runlia
0302 701E 06A0  32         bl    @filv                 ; Clear 12K VDP memory instead
     7020 60D8 
0303 7022 0000             data  >0000,>00,>3fff       ; of 16K, so that PABs survive
     7024 0000 
     7026 3FFF 
0308 7028 06A0  32 runlia  bl    @filv
     702A 60D8 
0309 702C 0FC0             data  pctadr,spfclr,16      ; Load color table
     702E 00C1 
     7030 0010 
0310               *--------------------------------------------------------------
0311               * Check if there is a F18A present
0312               *--------------------------------------------------------------
0316 7032 06A0  32         bl    @f18unl               ; Unlock the F18A
     7034 635E 
0317 7036 06A0  32         bl    @f18chk               ; Check if F18A is there
     7038 6378 
0318 703A 06A0  32         bl    @f18lck               ; Lock the F18A again
     703C 636E 
0320               *--------------------------------------------------------------
...
0330               *--------------------------------------------------------------
0331 703E 06A0  32 runlic  bl    @vidtab               ; Load video mode table into VDP
     7040 6142 
0332 7042 608A             data  spvmod                ; Equate selected video mode table
0333 7044 0204  20         li    tmp0,spfont           ; Get font option
     7046 000C 
0334 7048 0544  14         inv   tmp0                  ; NOFONT (>FFFF) specified ?
0335 704A 1304  14         jeq   runlid                ; Yes, skip it
0336 704C 06A0  32         bl    @ldfnt
     704E 61AA 
0337 7050 1100             data  fntadr,spfont         ; Load specified font
     7052 000C 
0338               *--------------------------------------------------------------
0339               * Did a system crash occur before runlib was called?
0340               *--------------------------------------------------------------
0341 7054 0280  22 runlid  ci    r0,>4a4a              ; Crash flag set?
     7056 4A4A 
0342 7058 1602  14         jne   runlie                ; No, continue
0343 705A 0460  28         b     @crash_handler.main   ; Yes, back to crash handler
     705C 605C 
0344               *--------------------------------------------------------------
0345               * Branch to main program
0346               *--------------------------------------------------------------
0347 705E 0262  22 runlie  ori   config,>0040          ; Enable kernel thread (bit 9 on)
     7060 0040 
0348 7062 0460  28         b     @main                 ; Give control to main program
     7064 7066 
**** **** ****     > fio.asm.2708
0071               *--------------------------------------------------------------
...
0090               * Main
0091               ********@*****@*********************@**************************
0092 7066 06A0  32 main    bl    @putat
     7068 627A 
0093 706A 0000             data  >0000,msg
     706C 70D0 
0094               
0095 706E 06A0  32         bl    @putat
     7070 627A 
0096 7072 0100             data  >0100,fname
     7074 70BF 
0097               
0098                       ;------------------------------------------------------
0099                       ; Prepare VDP for PAB and page out scratchpad
0100                       ;------------------------------------------------------
0101 7076 06A0  32         bl    @cpym2v
     7078 6282 
0102 707A 0200             data  vpab,pab,25           ; Copy PAB to VDP
     707C 70B6 
     707E 0019 
0103               
0104 7080 06A0  32         bl    @mem.scrpad.pgout     ; Page out scratchpad memory
     7082 6D38 
0105 7084 A000                   data  >a000           ; Memory destination @>a000
0106               
0107                       ;------------------------------------------------------
0108                       ; Open file
0109                       ;------------------------------------------------------
0110 7086 06A0  32         bl    @file.open
     7088 6E70 
0111 708A 0200             data  vpab                  ; Pass file descriptor to DSRLNK
0112 708C 21A0  38         coc   @wbit2,tmp2           ; Equal bit set?
     708E 6042 
0113 7090 130A  14         jeq   file.error            ; Yes, IO error occured
0114                       ;------------------------------------------------------
0115                       ; Read records
0116                       ;------------------------------------------------------
0117               readfile
0118 7092 06A0  32         bl    @file.record.read     ; Read record
     7094 6EAA 
0119 7096 0200             data  vpab
0120               
0121 7098 21A0  38         coc   @wbit2,tmp2           ; IO error occured?
     709A 6042 
0122 709C 1304  14         jeq   file.error            ; Yes, so handle file error
0123               
0124 709E 10F9  14         jmp   readfile              ; Next record
0125                       ;------------------------------------------------------
0126                       ; Close file
0127                       ;------------------------------------------------------
0128               close_file
0129 70A0 06A0  32         bl    @file.close           ; Close file
     70A2 6E8C 
0130 70A4 0200             data  vpab
0131               
0132               
0133                       ;------------------------------------------------------
0134                       ; Error handler
0135                       ;------------------------------------------------------
0136               file.error
0137 70A6 0984  56         srl   tmp0,8                ; Right align VDP PAB 1 status byte
0138 70A8 0284  22         ci    tmp0,io.err.eof       ; EOF reached ?
     70AA 0005 
0139 70AC 1302  14         jeq   exit_ok               ; All good. File closed by DSRLNK
0140 70AE 0460  28         b     @crash_handler        ; A File error occured. System crashed
     70B0 604C 
0141               
0142               
0143               exit_ok
0144 70B2 0420  54         blwp  @0
     70B4 0000 
0145               
0146               
0147               
0148               ***************************************************************
0149               * PAB for accessing file
0150               ********@*****@*********************@**************************
0151 70B6 0014     pab     byte  io.op.open            ;  0    - OPEN
0152                       byte  io.ft.sf.ivd          ;  1    - INPUT, VARIABLE, DISPLAY
0153 70B8 0300             data  vrecbuf               ;  2-3  - Record buffer in VDP memory
0154 70BA 5000             byte  80                    ;  4    - Record length (80 characters maximum)
0155                       byte  00                    ;  5    - Character count
0156 70BC 0000             data  >0000                 ;  6-7  - Seek record (only for fixed records)
0157 70BE 000F             byte  >00                   ;  8    - Screen offset (cassette DSR only)
0158               
0159               ;fname   byte  12                    ;  9    - File descriptor length
0160               ;        text 'DSK2.XBEADOC'         ; 10-.. - File descriptor (Device + '.' + File name)
0161               
0162               fname   byte  15                    ;  9    - File descriptor length
0163 70C0 ....             text 'DSK1.SPEECHDOCS'      ; 10-.. - File descriptor (Device + '.' + File name)
0164               
0165               
0166                       even
0167               
0168               
0169               msg
0170 70D0 152A             byte  21
0171 70D1 ....             text  '* File reading test *'
0172                       even
0173               
0174               
0175               
